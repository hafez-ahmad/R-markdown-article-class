...
b10_11_k1[[1]][1] # b10_k1
b10_11_k1[[1]][2] #b11_k1
...
#tempK10<-calc(toa_band10,fun=function(x){b10_11_k2[[1]][1]/log(b10_11_k1[[1]][1]/x+1)})
#tempK11<-calc(toa_band11,fun=function(x){b10_11_k2[[1]][2]/log(b10_11_k1[[1]][2]/x+1)})
...
allndvi<- raster::brick(ndvi1,ndvi2,ndvi3,ndvi4)
names(allndvi)<-c("Feb,2020","Feb,2021","Jun 2021","Mar,2022")

## Set up color gradient with 100 values between 0.0 and 1.0
breaks <- seq(-1, 1, by = 0.1)
cols <- colorRampPalette(c("red", "yellow", "green"))(length(breaks) - 1)

## Use `at` and `col.regions` arguments to set the color scheme
ndviplot<-levelplot(allndvi, at = breaks, col.regions = cols,layout=c(2, 2),
          ylab=list('Latitude',fontface='bold'),
          xlab=list('Longitude',fontface='bold'),margin=list(draw=FALSE,scales=list(draw=FALSE)))
